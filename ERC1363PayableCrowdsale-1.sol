...
contract TicketERC20 is Context, Pausable, IERC20, IERC1363, ERC165 {
    using SafeMath for uint256;
    using Address for address;
  ...
  modifier onlyAfterDeadline()  {
     ...
     * construction.
     */
    constructor (string memory name, string memory symbol, address _reserve, uint256 expiryDate_) public {
        require(_reserve != address(0));
        require(msg.sender != address(0));
        require(expiryDate_ > now);
        ...
        _registerInterface(_INTERFACE_ID_ERC1363_TRANSFER);
        _registerInterface(_INTERFACE_ID_ERC1363_APPROVE);
        emit Transfer(msg.sender, _reserve, _totalSupply);
    }
     ...
     * - the caller must have a balance of at least `amount`.
     */
    function transfer(address recipient, uint256 amount) public onlyAdminTransfer virtual override returns (bool) {
     ...
     * `amount`.
     */
    function transferFrom(address sender, address recipient, uint256 amount) public onlyAdminTransfer virtual override returns (bool) {
        _transfer(sender, recipient, amount);
        _approve(sender, _msgSender(), _allowances[sender][_msgSender()].sub(amount, "ERC20: transfer amount exceeds allowance"));
        return true;
    }
     ...
     * - `spender` cannot be the zero address.
     */
    function increaseAllowance(address spender, uint256 addedValue) public onlyAdminTransfer virtual returns (bool) {
        _approve(_msgSender(), spender, _allowances[_msgSender()][spender].add(addedValue));
        return true;
    }
     ...
     * `subtractedValue`.
     */
    function decreaseAllowance(address spender, uint256 subtractedValue) public onlyAdminTransfer virtual returns (bool) {
        _approve(_msgSender(), spender, _allowances[_msgSender()][spender].sub(subtractedValue, "ERC20: decreased allowance below zero"));
        return true;
    }

    function switchAdmin(address newAdmin) public onlyAdminTransfer virtual returns (bool) {
        require(msg.sender != address(0));
        admin = newAdmin;
        return true;
     ...
     * - `sender` must have a balance of at least `amount`.
     */
    function _transfer(address sender, address recipient, uint256 amount) internal onlyAdminTransfer virtual {
        require(sender != address(0), "ERC20: transfer from the zero address");
        require(recipient != address(0), "ERC20: transfer to the zero address");
     ...
     * - `to` cannot be the zero address.
     */
    function _mint(address account, uint256 amount) internal onlyAdminTransfer virtual {
        require(account != address(0), "ERC20: mint to the zero address");
     ...
     * - `account` must have at least `amount` tokens.
     */
    function _burn(address account, uint256 amount) internal onlyAdminTransfer onlyAfterDeadline virtual {
        require(account != address(0), "ERC20: burn from the zero address");
     ...
     * - `spender` cannot be the zero address.
     */
    function _approve(address owner, address spender, uint256 amount) internal onlyAdminTransfer virtual {
        require(owner != address(0), "ERC20: approve from the zero address");
        require(spender != address(0), "ERC20: approve to the zero address");
 ...
 * behavior.
 */
contract ERC1363PayableCrowdsale is ERC1363Payable, ReentrancyGuard, Pausable {
    using SafeMath for uint256;
